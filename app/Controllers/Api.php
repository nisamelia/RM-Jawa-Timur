...
use CodeIgniter\RESTful\ResourceController; //api harus ada ini, dibuat otomatis
...
class Api extends ResourceController
{
    ...
    public function polyline()
    {
        ...
        ST_Length(ST_Transform(ST_SetSRID(ST_AsText(geom),4326),32648)) AS panjang,                                                                 
        nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polyline2");
        $query_array = $query->getResultArray();
        $feature = array();
        foreach ($query_array as $q) {
            $feature[] = [
    ...
    {
        $db = db_connect();
        $query = $db->query("SELECT id, ST_AsGeoJSON(geom) AS geom, ST_Area(ST_Transform(ST_SetSRID(ST_AsText(geom),4326),32648)) AS area, nama, deskripsi, foto, created_at, updated_at FROM tbl_data_polygon");
        $query_array = $query->getResultArray();
        $feature = array();
        foreach ($query_array as $q) {
            $feature[] = [
